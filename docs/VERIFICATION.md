...
| Automated tests | `npm test` passes 26 tests across 6 files. | Passed |
...
| Webhook idempotency and events | A full-handler integration test sends the same correctly signed raw subscription event twice, observes `processed` then `duplicate`, and confirms the duplicate performs only the event-ID insert attempt. Event handlers cover checkout, subscription created/updated/deleted, invoice paid, and invoice failed. |
